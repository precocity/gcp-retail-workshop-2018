CREATE TEMP FUNCTION ltv(cust_transactions ARRAY<STRUCT<order_id INT64, channel_id INT64, total_amount FLOAT64, store_id STRING, transaction_date DATE>>, contribution_margin FLOAT64, retention_rate FLOAT64,discount_rate FLOAT64,retention_costs FLOAT64,time_horizon_years INT64)
RETURNS FLOAT64
LANGUAGE js AS """
...
cust_rels as (
  SELECT rel.source_customer_id as customer_id, ARRAY_AGG(STRUCT(rel.relationship_type as  relationship_type, rel.target_customer_id as related_customer_id)) as relationships
...
cust_sales as (
  ...
  FROM `retail_demo_warehouse.sales_events` sales
  GROUP BY 1
)
...
ON customer.customer_id = CAST(sales.customer_id as INT64)
